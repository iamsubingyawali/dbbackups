...
function Send-ToDatabase([string]$Name, [string]$FileName, [string]$FileSize, [string]$Type, [string]$Status, [string]$Message) {
    if ($SEND_LOGS){
        ...
        sqlcmd -S "$MSSQL_REMOTE_HOST,$MSSQL_REMOTE_PORT" -U $MSSQL_REMOTE_USER -P $MSSQL_REMOTE_PASS -d $MSSQL_REMOTE_DB -Q $QUERY | Out-File -Append "$LOG_BASE_DIR/mssql_backup_record.log"

        # Check exit status and add logs
        if (!$?){
            "## $TIMESTAMP $DB Error: Could not send backup records to remote database. Please check your remote database configurations." | Out-File -Append "$LOG_BASE_DIR/mssql_backup_record.log"
        }
        else{
            "## $TIMESTAMP $DB Backup details sent to remote database." | Out-File -Append "$LOG_BASE_DIR/mssql_backup_record.log"
        }
    }
}
...
if ($TYPE -eq "full"){
    foreach ($DB in $DATABASES.Keys) {
        ...
        if (!$?){
            # Add log
            "## $TIMESTAMP $DB Error: Could not create a backup directory." | Out-File -Append "$LOG_BASE_DIR/mssql_full_backup.log"
            # Send to database
        ...
        if (!$?){
            # Add log
            "## $TIMESTAMP $DB Error: Could not perform a database backup. Please check your database and server configurations." | Out-File -Append "$LOG_BASE_DIR/mssql_full_backup.log"
            # Send to database
        ...
        if ($S3_BUCKET_NAME -ne "" -and $SYNC_TO_S3) {
            ...
            if (!$?){
                # Add log
                "## $TIMESTAMP $DB Error: Could not upload backups to S3. Please check your AWS CLI and S3 bucket configurations." | Out-File -Append "$LOG_BASE_DIR/mssql_full_backup.log"
                # Send to database
        ...
        if ($SYNC_TO_GDRIVE) {
            ...
            if (!$?){
                # Add log
                "## $TIMESTAMP $DB Error: Could not upload backups to Google Drive. Please check your Google Drive and gdrive tool configurations." | Out-File -Append "$LOG_BASE_DIR/mssql_full_backup.log"
                # Send to database
...
elseif ($TYPE -eq "audit") {
    foreach ($DB in $DATABASES.Keys) {
        ...
        if (Test-Path $AUDIT_FOLDER) {
            ...
            if (!$?){
                # Add log
                "## $TIMESTAMP $DB Error: Could not create a backup directory." | Out-File -Append "$LOG_BASE_DIR/mssql_audit_backup.log"
                # Send to database
            ...
            $S3_BUCKET_NAME = $DATABASES[$DB]
            if ($S3_BUCKET_NAME -ne "" -and $SYNC_TO_S3) {
                aws s3 sync $BACKUP_DIR "s3://$S3_BUCKET_NAME" | Out-File -Append "$LOG_BASE_DIR/mssql_audit_backup.log"
                # Check exit status
                if (!$?){
                    # Add log
                    "## $TIMESTAMP $DB Error: Could not upload backups to S3. Please check your AWS CLI and S3 bucket configurations." | Out-File -Append "$LOG_BASE_DIR/mssql_audit_backup.log"
                    # Send to database
            ...
            if ($SYNC_TO_GDRIVE) {
                ...
                if (!$?){
                    # Add log
                    "## $TIMESTAMP $DB Error: Could not upload backups to Google Drive. Please check your Google Drive and gdrive tool configurations." | Out-File -Append "$LOG_BASE_DIR/mssql_audit_backup.log"
                    # Send to database
    ...
    foreach ($DB in $DATABASES.Keys) {
        ...
        if (!$?){
            # Add log
            "## $TIMESTAMP $DB Error: Could not create a backup directory." | Out-File -Append "$LOG_BASE_DIR/mssql_diff_backup.log"
            # Send to database
        ...
        if (!$?){
            # Add log
            "## $TIMESTAMP $DB Error: Could not perform a database backup. Please check your database and server configurations." | Out-File -Append "$LOG_BASE_DIR/mssql_diff_backup.log"
            # Send to database
